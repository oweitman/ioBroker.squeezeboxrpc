...
function ioSBPlayer(server,playerdata) {
    ...
    this.artisthandling = this.adapter.config.artisthandling || false;
    
    this.FORBIDDEN_CHARS = /[^\d\w\_]+/gm;
            ...
            for (const key in this.sbPlayerStatusLoop) {
                ...
                if (playerdata.hasOwnProperty(key)) {
                    ...
                    if (key == 'artist' && !playerdata['artisthandling'] && this.artisthandling==true) {
                        ...
                        if (playerdata.hasOwnProperty('artist')) {
                            value = playerdata['artist'];
                        }
                        if (playerdata.hasOwnProperty('composer')) {
                            value = playerdata['composer'];
                        }
                        if (playerdata.hasOwnProperty('conductor')) {
                            value = playerdata['conductor'];
                        }
                        if (playerdata.hasOwnProperty('band')) {
                            value = playerdata['band'];
                        }
                        if (playerdata.hasOwnProperty('albumartist')) {
                            value = playerdata['albumartist'];
                        }
                        if (playerdata.hasOwnProperty('trackartist')) {
                    ...
                    if (key == 'artist' && (playerdata.hasOwnProperty('albumartist')||"")=="") {
                        value = '';
                    } else {
                        value = playerdata['albumartist'];                        
                    }
                    if (key == 'albumartist' && (playerdata.hasOwnProperty('artist')||"")=="") {
                        value = '';
                    } else {
                        value = playerdata['artist'];
                    }
    ...
    this.doPlayerStatus = function(playerdata){
        ...
        if (playerdata.name)        this.playername = playerdata.name.replace(this.FORBIDDEN_CHARS, '_');
