...
const { Announcement } = require('./announce');
 ...
 * @param i18n - Translation object for internationalization.
 *
 * Properties include:
 ...
 */
function ioSBPlayer(server, playerdata, i18n) {
    this.server = server;
    this.i18n = i18n;
    ...
    this.ioUtil.addTranslations(this.i18n, this.sbPlayerStatusMain, 'psm');
    this.ioUtil.addTranslations(this.i18n, this.sbPlayerStatusLoop, 'psl');
    this.ioUtil.addTranslations(this.i18n, this.sbPlayerStatusPlaylist, 'psp');
    this.ioUtil.addTranslations(this.i18n, this.sbPlayerButtons, 'pb');
    ...
    this.init = async function (playerdata) {
        ...
        this.ioUtil.setMyTimeout('player', this.doObserverPlayer.bind(this), delay * 190);
    };
    ...
    this.doObserverPlayer = function () {
        ...
        this.ioUtil.setMyTimeout('player', this.doObserverPlayer.bind(this), this.adapter.config.playerrefresh);
    };
    /**
    ...
    this.doPlayerUpdateStatus = result => {
        ...
        if (playerdata) {
            for (const key in this.sbPlayerStatusLoop) {
                let value = '';
                if (key == 'artwork_url' || key == 'coverid' || key == 'artist') {
                    continue;
                }
                if (Object.prototype.hasOwnProperty.call(playerdata, key)) {
                    value = playerdata[key];
            ...
            const hasCoverId = playerdata.coverid && playerdata.coverid[0] != '-';
            if (hasCoverId || Object.prototype.hasOwnProperty.call(playerdata, 'artwork_url') || fullStatus) {
                const artworkUrl = hasCoverId
                    ? this.createArtworkUrl(playerdata.coverid)
                    : this.normalizeArtworkUrl(playerdata.artwork_url || '');
                this.setState(
                    this.sbPlayerStatusLoop.artwork_url.name,
                    this.convertState(this.sbPlayerStatusLoop.artwork_url, artworkUrl),
                    this.statePath,
                    this.playername,
                );
            }

            const artist = playerdata.artist || playerdata.trackartist || '';
            if (
                Object.prototype.hasOwnProperty.call(playerdata, 'artist') ||
                Object.prototype.hasOwnProperty.call(playerdata, 'trackartist') ||
                fullStatus
            ) {
                this.setState(
                    this.sbPlayerStatusLoop.artist.name,
                    this.convertState(this.sbPlayerStatusLoop.artist, artist),
                    this.statePath,
                    this.playername,
                );
            }
    ...
    this.doStateChange = function (idParts, state) {
        ...
        idParts.shift();
        if (idParts[0] == 'Announce') {
            if (state.val !== ' ') {
                this.setState(idParts[0], ' ', this.statePath, this.playername, false);
                this.announcement.start(state.val).catch(error => {
                    const message = error instanceof Error ? error.message : String(error || 'Unknown LMS error');
                    this.ioUtil.logerror(`Announce failed for ${this.playername}: ${message}`);
                });
            }
        }
        if (idParts[0] == 'Volume') {
    ...
    this.normalizeArtworkUrl = function (artworkUrl) {
        const regex = /imageproxy\/(.*)\/[^/]/gm;
        const match = regex.exec(artworkUrl);
        if (match && match.index > 0) {
            return decodeURIComponent(match[1]);
        }
        if (['html/images/radio.png', 'html/images/cover.png', 'html/images/favorites.png'].includes(artworkUrl)) {
            return `http://${this.adapter.config.server}:${this.adapter.config.port}/${artworkUrl}`;
        }
        return artworkUrl;
    };
    this.disconnect = () => {
        ...
        this.ioUtil.clearTimeout('player');
        this.connected = 0;
        this.setState('Power', 0, this.statePath, this.playername, false);
        this.setState('Connected', 0, this.statePath, this.playername, false);
    };
    /**
