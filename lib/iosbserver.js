    ...
    this.doSyncgroups = function (result) {
        ...
        if (result.result && result.result.syncgroups_loop) {
            ...
            if (result.result?.syncgroups_loop?.[0]?.sync_member_names) {
                result.result.syncgroups_loop[0].sync_member_names = result.result?.syncgroups_loop?.[0]?.sync_member_names.split(",").map(el => this.sanitizePlayername(el));
            }
            this.createObject(stateTemplate, this.ServerStatePath, null,
                () => this.setState(this.sbServerStatus[key].name, JSON.stringify(result.result.syncgroups_loop), this.ServerStatePath)
            );
        }
    };
