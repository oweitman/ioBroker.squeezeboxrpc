...
const { createDiscoveryRequest, parseDiscoveryResponse } = require('./lms/discovery');
...
function IoSbServer(adapter, i18n) {
    ...
    this.discoverLMS = function (command, message, from, callback) {
        ...
        const data = [];
        for (const [address, srv] of Object.entries(this.otherserver)) {
            if ((Date.now() - srv.TIMESTAMP) / 1000 > 60) {
                delete this.otherserver[address];
            } else {
    ...
    this.getDiscoverServers = () => {
        ...
        this.server.on('message', async (data, rinfo) => {
            this.ioUtil.logsilly(`getDiscoverServers: message received from ${rinfo.address}`);
            const srv = parseDiscoveryResponse(data, rinfo.address);
            if (srv) {
                this.otherserver[rinfo.address] = srv;
                const state = {};
                Object.assign(state, this.sbServerStatus['otherServers']);
                state.name = rinfo.address.replace(/\./g, '-');
        ...
        this.ioUtil.setMyTimeout('discover', this.doDiscoverServerSearch, 1000);
    };
    /**
    ...
    this.doDiscoverServerSearch = () => {
        ...
        const msg = createDiscoveryRequest();
