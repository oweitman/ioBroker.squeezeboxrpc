...
const { createLmsClient } = require('./lms/client');
const { LmsTelnetEventClient } = require('./lms/telnetEventClient');
...
function IoSbServer(adapter, i18n) {
    ...
    this.doTelnet = () => {
        ...
        const refreshCommands = new Set(['client reconnect', 'client disconnect', 'client new', 'power 0', 'power 1']);
        this.telnetEventClient = new LmsTelnetEventClient({
            host: this.adapter.config.server,
            port: Number.parseInt(this.adapter.config.telnetport),
            username: this.adapter.config.username,
            password: this.adapter.config.password,
            logDebug: message => this.ioUtil.logdebug(message),
            onEvent: event => {
                if (refreshCommands.has(event.command)) {
                    this.ioUtil.logdebug(`doTelnet received ${event.command} for : ${event.playerId}`);
                    this.getServerstatus();
                }
            },
        });
        this.telnetEventClient.start();
    };
    /**
