...
const { createLmsConnection } = require('./lms/connection');
const { LmsDiscovery } = require('./lms/discovery');
const ioSBPlayer = require(`${__dirname}/iosbplayer`);

const SERVER_REFRESH_EVENTS = new Set(['client reconnect', 'client disconnect', 'client new', 'power 0', 'power 1']);
const PLAYER_REFRESH_EVENTS = new Set(['playlist', 'newmetadata', 'mode', 'play', 'pause', 'stop', 'power', 'mixer']);
const FULL_PLAYER_REFRESH_EVENTS = new Set(['playlist', 'newmetadata']);

/**
 * Returns the names of all initialized players.
 *
 * @param players - Player collection indexed by player ID.
 * @returns The initialized player names in registration order.
 */
function getPlayerNames(players = {}) {
    return Object.values(players)
        .map(player => player?.playername)
        .filter(playername => typeof playername === 'string' && playername.length > 0);
}
 ...
 * @param i18n    - An instance of the i18n module for internationalization.
 */
function IoSbServer(adapter, i18n) {
    ...
    this.ioUtil.addTranslations(this.i18n, this.sbServerStatus, 'ss');
    this.ioUtil.addTranslations(this.i18n, this.sbFavoritesState, 'fs');
    ...
    this.currentStates = {};
    this.players = [];
    this.pendingFullPlayerRefresh = new Set();
    this.adapter = adapter;

    this.adapter.setState('info.connection', false, true);
    this.adapter.subscribeStates('*');

    this.sbServer = createLmsConnection({
        host: this.adapter.config.server,
        port: Number.parseInt(this.adapter.config.port),
        username: this.adapter.config.username,
        password: this.adapter.config.password,
        connectionType: this.adapter.config.connectionType || 'jsonrpc',
        webSocketUrl: this.adapter.config.webSocketUrl,
        useTelnet: this.adapter.config.usetelnet,
        telnetPort: Number.parseInt(this.adapter.config.telnetport),
        setTimeout: this.adapter.setTimeout.bind(this.adapter),
        clearTimeout: this.adapter.clearTimeout.bind(this.adapter),
        logDebug: message => this.ioUtil.logdebug(message),
        logError: message => this.ioUtil.logerror(message),
        onEvent: event => {
            if (SERVER_REFRESH_EVENTS.has(event.command)) {
                this.ioUtil.logdebug(`received LMS event ${event.command} for : ${event.playerId}`);
                this.getServerstatus();
            }
            const commandName = event.command.split(' ')[0];
            if (event.playerId && PLAYER_REFRESH_EVENTS.has(commandName)) {
                if (FULL_PLAYER_REFRESH_EVENTS.has(commandName)) {
                    this.pendingFullPlayerRefresh.add(event.playerId);
                }
                this.ioUtil.setMyTimeout(
                    `player-event-${event.playerId}`,
                    () => {
                        const forceFull = this.pendingFullPlayerRefresh.delete(event.playerId);
                        this.players[event.playerId]?.getPlayerUpdateStatus(forceFull);
                    },
                    100,
                );
            }
    ...
    this.runBackground = function (operation, description) {
        void operation.catch(error => {
            if (!this.ioUtil.doClose) {
                this.ioUtil.logerror(`${description}: ${error instanceof Error ? error.message : String(error)}`);
            }
        });
    };
    /**
    ...
    this.doObserverServer = function () {
        ...
        this.ioUtil.setMyTimeout(
            'serverstatus',
            this.doObserverServer.bind(this),
            this.adapter.config.serverrefresh * 1000,
        );
    };
    /**
    ...
    this.processMessages = function (msg) {
        ...
        if (msg.command === 'getPlayerNames') {
            this.ioUtil.logdebug('send getPlayerNames');
            this.sendPlayerNames(msg.command, msg.from, msg.callback);
        }
    };

    /**
     * Sends the names of all initialized players to a SendTo caller.
     *
     * @param command - The SendTo command identifier.
     * @param from - The sender of the message.
     * @param callback - The callback token supplied by ioBroker.
     */
    this.sendPlayerNames = (command, from, callback) => {
        this.adapter.sendTo(from, command, getPlayerNames(this.players), callback);
    };
    ...
    this.discoverLMS = function (command, message, from, callback) {
        ...
        const data = (this.discovery?.getServers() || []).map(srv => ({
            label: `${srv.NAME}/${srv.ADDRESS}`,
            value: srv.ADDRESS,
        }));
    ...
    this.doObserverFavorites = function () {
        ...
        this.runBackground(this.delFavorites(), 'Cannot delete favorites');
        if (this.adapter.config.usefavorites) {
            this.runBackground(this.setFavorites(), 'Cannot update favorites');
            return;
        }
        this.ioUtil.setMyTimeout(
            'favorites',
    ...
    this.getDiscoverServers = () => {
        ...
        this.discovery = new LmsDiscovery({
            intervalMs: this.adapter.config.discoveryrefresh * 1000,
            schedule: (callback, delay) => this.ioUtil.setMyTimeout('discover', callback, delay),
            cancelSchedule: () => this.ioUtil.clearTimeout('discover'),
            logDebug: message => this.ioUtil.logdebug(message),
            onServer: async srv => {
                const address = String(srv.ADDRESS);
                const state = {};
                Object.assign(state, this.sbServerStatus['otherServers']);
                state.name = address.replace(/\./g, '-');
            ...
            onExpired: srv => {
                ...
                this.ioUtil.logdebug(`Autodiscover: Server removed ${srv.NAME} IP: ${address}`);
            },
            onError: err => this.ioUtil.logerror(`Error with Server discovery ${err.message}`),
        });
    ...
    this.closeConnections = function () {
        ...
        for (const key in this.players) {
            this.players[key].ioUtil.closeConnections();
        }
        if (this.adapter.config.usediscovery) {
            this.doDiscoverServerClose();
        }
    ...
    this.setFavorites = async function () {
        ...
        this.getFavoritesLMS('', result => {
            if (!this.ioUtil.doClose) {
                this.runBackground(this.setFavoritesDP(result), 'Cannot create favorites');
            }
        });
    };
    ...
    this.setFavoritesDP = async function (favorites) {
        ...
        for (const favkey in favorites) {
            ...
            if (favorite['hasitems'] > 0) {
                this.getFavoritesLMS(oid, result => {
                    if (!this.ioUtil.doClose) {
                        this.runBackground(this.setFavoritesDP(result), 'Cannot create nested favorites');
                    }
                });
            }
        }
    ...
    this.checkNewPlayer = function (playersdata) {
        ...
        for (const key in playersdata) {
            if (!Object.prototype.hasOwnProperty.call(this.players, playersdata[key].playerid)) {
                this.players[playersdata[key].playerid] = new ioSBPlayer(this, playersdata[key], this.i18n);
            }
        }
    };
    /**
    ...
    this.checkTPE2 = function () {
        this.request('', ['pref', 'useTPE2AsAlbumArtist', '?'], result => {
            if (result.ok) {
     ...
     * It returns early if id, state, or state.ack is falsy.
     */
    this.doServerStateChange = async function (idParts, state) {
        ...
        if (idParts[0] == 'cmdGeneral') {
            if (state.val !== ' ') {
                try {
                    ...
                    await this.requestAsync(cmd[0], cmd[1]);
                } catch {
                    this.ioUtil.logerror(`cmdGeneral, Parameter error: ${state.val}`);
                }
                this.setState(idParts[0], ' ', this.ServerStatePath, null, false);
            }
        }
    };
    /**
        ...
        return new Promise((resolve, reject) => {
            this.sbServer.request(playerid, params, result => {
                if (result.ok) {
                    ...
                    const message = result && (result.message || result.error || result.errorMessage);
                    reject(new Error(typeof message === 'string' ? message : 'LMS request failed'));
                }
            });
        });
    };
    ...
    this.disconnect = () => {
        ...
        this.firstStart = false;
        this.connected = 0;
        this.ioUtil.clearTimeout('serverstatus');
        this.ioUtil.clearTimeout('favorites');
        if (this.connected == 0 && this.ioUtil.observers['checkserver']) {
            return;
        }
        ...
        this.ioUtil.setMyTimeout('checkserver', this.doCheckServer, 10 * 1000);
    };
    /**
    ...
    this.connect = () => {
        ...
        this.ioUtil.clearTimeout('checkserver');
        this.setState('connection', true, 'info');
        this.connected = 1;
        this.init();
    };
     ...
     */
    this.doCheckServer = async () => {
        this.ioUtil.logdebug('doCheckServer');
        try {
            await this.requestAsync('', ['serverstatus', '0', '888']);
            this.connect();
        } catch {
            if (!this.connected) {
                this.ioUtil.setMyTimeout('checkserver', this.doCheckServer, 10 * 1000);
    ...
    this.createFolder = async function (foldername, level1path, level2path) {
        await this.ioUtil.createFolderNotExistsAsync(foldername, level1path, level2path);
    };
    this.createDevice = async function (devicename, level1path, level2path) {
        await this.ioUtil.createDeviceNotExistsAsync(devicename, level1path, level2path);
    };
...
module.exports.getPlayerNames = getPlayerNames;
