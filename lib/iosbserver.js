...
function IoSbServer(adapter) {
    ...
    this.doObserverServer = function () {
        this.logsilly('doObserverServer');
    ...
    this.doObserverFavorites = function () {
        this.logsilly('doObserverFavorites');
    ...
    this.getDiscoverServers = () => {
        this.logsilly('getDiscoverServers');
        const that = this;
        ...
        this.server.on('error', function (err) {
            that.log.error("Error with Server discovery " + err.message);
            if (err.message.includes("EADDRINUSE")) {
                that.log.info("use 'lsof -i -P' to check for ports used.");
            }
            try {
                ...
                that.log.error("server.close() " + err.message);
            }
        });
        setTimeout(this.doDiscoverServerSearch, 1000);
    };
    this.doDiscoverServerSearch = () => {
        this.logsilly('doDiscoverServerSearch');
        const that = this;
    ...
    this.newgetFavoritesLMS = function (id = '', callback) {
        this.logsilly('newgetFavoritesLMS');
    ...
    */
    ...
    this.checkServerstatusStates = function (result) {
        this.logsilly('checkServerstatusStates');
        for (const key in this.sbServerStatus) {
            if (Object.prototype.hasOwnProperty.call(result.result, key)) {
    ...
    this.createObject = function (stateTemplate, level1path, level2path, callback) {
        ...
        this.logdebug('createObject ' + name);
