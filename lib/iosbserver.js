...
function IoSbServer(adapter) {
    ...
    this.doTelnet = function() {        
        ...
        this.telnet.on('data', (data) => {
            this.log.debug('doTelnet recieved Data: ' + data.toString());
            const regex = /^((?:[0-9A-Fa-f]{2}[:-]){5}(?:[0-9A-Fa-f]{2}))\s*(.*)/gm;
            let m;
            if ((m = regex.exec(decodeURIComponent(data.toString()))) !== null) {
                if (m[2] == 'client reconnect') {
                    this.log.debug('doTelnet recieved reconnect for : ' + m[1]);
                    this.getServerstatus();
                }
                if (m[2] == 'client disconnect') {
                    this.log.debug('doTelnet recieved disconnect for : ' + m[1]);
                    this.getServerstatus();
                }
                if (m[2] == 'client new') {
                    this.log.debug('doTelnet recieved disconnect for : ' + m[1]);
                ...
                if (m[2] == 'power 0') {
                    this.log.debug('doTelnet recieved power off : ' + m[1]);
                    this.getServerstatus();
                }
                if (m[2] == 'power 1') {
                    this.log.debug('doTelnet recieved power on : ' + m[1]);
                    this.getServerstatus();
                }
            }
        });
