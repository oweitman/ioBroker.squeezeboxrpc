...
class ioUtil {
    ...
    /**
     * Asynchronously creates a device object in the ioBroker system, if it does not already exist.
     *
     * @param devicename - The name of the device to create.
     * @param level1path - The first level path to prepend to the device name, can be null or empty.
     * @param level2path - The second level path to prepend to the device name, can be null or empty.
     * @returns A promise that resolves when the device object is set in the adapter.
     */
    createDeviceNotExistsAsync(devicename, level1path, level2path) {
        this.logdebug(`createDeviceNotExistsAsync ${devicename}`);
        const name = (level1path ? `${level1path}.` : '') + (level2path ? `${level2path}.` : '') + devicename;
        const newobj = {
            type: 'device',
            common: {
                name: devicename,
    ...
    setMyTimeout(id, callback, time, arg1 = null, arg2 = null) {
        ...
        if (!Number.isFinite(timeout) || timeout < 0) {
            this.logerror(`Cannot schedule timeout ${id}: invalid delay ${JSON.stringify(time)}`);
            return;
        }
        this.clearTimeout(id);
        this.observers[id] = this.adapter.setTimeout(callback.bind(this), timeout, arg1, arg2);
    }
    /**
    ...
    /**
     * Adds translated descriptions to an object.
     *
     * @param i18n - The i18next instance to use for translation.
     * @param obj - The object to add translated descriptions to.
     * @param prefix - The prefix to use to form the translation key.
     */
    addTranslations(i18n, obj, prefix) {
        for (const key in obj) {
            const translateObj = i18n.getTranslatedObject(`${prefix}_${key}`);
            obj[key].desc = translateObj;
        }
    }
    /**
