          ...
          vis.conn.getStates(
            ...
            function(err, obj) {
              ...
              if (!data.viewindex || data.viewindex.trim() == "") {
                data.viewindex = this.getViewindex(favorites2).join(", ");
              }
              if (vis.editMode && data.bCount != Math.min(favorites2.length, data.viewindex.split(",").length)) {
                data.bCount = Math.min(favorites2.length, data.viewindex.split(",").length);
...
`;
              ...
              for (let i = 0; i < viewindex.length; i++) {
                const favorite = this.findById(favorites2, viewindex[i]);
                text += "  <div>";
        ...
        findById: function(favorites2, id) {
          return favorites2.find(
            function(cur) {
              return cur.id.trim() == this.trim();
            }.bind(id)
          );
        },
        getViewindex: function(favorites2) {
          return favorites2.map((cur) => cur.id);
        },
        checkViewindexExist: function(viewindex, favorites2) {
          return viewindex.map(function(item) {
            return favorites2.find((el) => el.id == item) ? item : "0";
          });
        }
      };
    }
        ...
        createWidget: function(widgetID, view, data, style) {
          return __async(this, null, function* () {
            ...
            const renderPlayers = function(playerNames) {
              ...
              const buttonmargin = data.buttonmargin || "0px";
              if (!data.viewindex || data.viewindex.trim() == "") {
                data.viewindex = this.getViewindex(players2).join(", ");
              }
              data.defaultPlayer = data.defaultPlayer || Object.keys(players2)[0] || "0";
              if (vis.editMode && data.bCount != Math.min(players2.length, data.viewindex.split(",").length)) {
                data.bCount = Math.min(players2.length, data.viewindex.split(",").length);
                redrawinspectwidgets = true;
              }
              const viewindex = data.viewindex.split(", ");
              if (data.formattype == "formatselect") {
                ...
                for (let i = 0; i < viewindex.length; i++) {
                  ...
                  buttonsText = buttonsText.trim() != "" ? buttonsText : players2[viewindex[i]];
                  if (vis.editMode && editmodehelper) {
                    buttonsText += ` [${viewindex[i]}]`;
                  }
                  option += `<option value="${players2[viewindex[i]]}">${buttonsText}</option>`;
                }
...
`;
                ...
                for (let i = 0; i < viewindex.length; i++) {
                  text += "  <div >";
                  text += `    <input type="radio" id="${widgetID}${players2[viewindex[i]]}" name="${widgetID}" value="${players2[viewindex[i]]}" ${viewindex[i] == data.defaultPlayer ? "checked" : ""}>`;
                  text += `    <label for="${widgetID}${players2[viewindex[i]]}">`;
                  text += "      <span>";
        ...
        checkViewindexExist: function(viewindex, players2) {
          return viewindex.map(function(item) {
...
`;
          ...
          $(`#${widgetID}`).html(text);
          for (let i = 0; i < players2.length; i++) {
            const elemp = $(`#${data.widgetPlayer} input[value="${players2[i]}"]  + label span :first-child`);
            const elems = $(`#${widgetID}${players2[i]} + label span canvas`);
    ...
    "squeezeboxrpc/js/squeezeboxrpc.js"(exports) {
      ...
      vis.binds["squeezeboxrpc"] = {
        ...
        setPlayersChanged: function($div, widgetPlayer, fdata, onChange_callback, boundstates_callback, playerChanged_callback) {
          ...
          $(".vis-view").off(`playerschanged.${fdata.widgetID}`).on(`playerschanged.${fdata.widgetID}`, `#${widgetPlayer}`, fdata, () => {
            bindPlayerStates();
